...
def get_style_suffix(series_index: int | None) -> set[str]:
    if series_index == 0:
        return {"anime sketch chibi drawing style, pastel background"}
    if series_index == 1:
        return {"anime sketch drawing style"}
    if series_index == 2:
...
def get_random_series_adjective(series_index: int | None) -> str:
    ...
    return pref + " " + random.choice(list(get_series_adjectives_set(series_index)))
...
def get_basic_subject():
    return CreatuteType("", [WEARING_ARMOR])
