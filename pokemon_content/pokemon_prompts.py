...
def get_visual_description(card: Pokemon) -> str:
    segments = []
...
def get_image_prompt(card: Pokemon):
...
def get_subject_description(card: Pokemon):
...
def get_detail_description(card: Pokemon) -> str:
...
def get_full_subject_description(card: Pokemon):
    subject_line = get_subject_description(card)
    subject_line += get_detail_description(card)
    return subject_line


def generate_card_name(card: Pokemon) -> str:
    ...
    print(prompt)
    response = gpt_client().get_completion(prompt, max_tokens=256, n=3)

    potential_names = set()
    for potential_name in response.choices:
        name = potential_name.message.content
        name = name.strip()
    ...
    filtered_names = set(potential_names)
...
def generate_desc(card: Pokemon) -> str:
