...
from util.img_util import b64_to_pil
...
def render_card(card: Pokemon):
    ...
    card_art_image = b64_to_pil(card.curr_image)
...
def render_weakness_and_resist(card: Pokemon, image: Image):
...
def render_status_element(card: Pokemon, image: Image, element: Element, x_position: int):
...
def card_from_json(data: dict) -> Pokemon:
    card = Pokemon(
        index=data["index"],
        name=data["name"],
